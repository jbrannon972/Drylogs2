...
export const FinalMoistureVerification: React.FC<FinalMoistureVerificationProps> = ({ job, onNext }) => {
  ...
  // DRW Decision Tree
  const [showDRWTree, setShowDRWTree] = useState(false);
  const [drwAnswers, setDrwAnswers] = useState({
    homeownerRequesting: false,
    risksExplained: false,
    waiverWillingness: false,
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* DRW Decision Tree - Shows when materials still wet */}
      {wetMaterials > 0 && allMaterialsVerified && (
        <div className="border-2 border-red-500 rounded-lg p-5 bg-red-50">
          <div className="flex items-start gap-3 mb-4">
            <AlertTriangle className="w-6 h-6 text-red-600 flex-shrink-0" />
            <div>
              <h4 className="font-semibold text-red-900 mb-1 text-lg">⚠️ Materials Still Wet - Pull Decision Required</h4>
              <p className="text-sm text-red-800">
                {wetMaterials} material{wetMaterials !== 1 ? 's are' : ' is'} still above acceptable moisture levels.
                Answer the questions below to determine if early removal is appropriate.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white border-2 border-red-400 rounded-lg p-4">
            <h5 className="font-semibold text-gray-900 mb-3">
              ...
              Early Equipment Pull Assessment
            </h5>
            <p className="text-sm text-gray-700 mb-4">
              Answer ALL questions to proceed. All must be YES to create a Drying Release Waiver (DRW):
            </p>
            ...
            <div className="space-y-3">
              <label className="flex items-start gap-3 p-3 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                <input
                  type="checkbox"
                  checked={drwAnswers.homeownerRequesting}
                  onChange={(e) => setDrwAnswers({ ...drwAnswers, homeownerRequesting: e.target.checked })}
                  className="w-5 h-5 mt-0.5 rounded border-gray-300"
                />
                <div>
                  <p className="font-medium text-gray-900">Is the homeowner requesting early removal?</p>
                  <p className="text-xs text-gray-600 mt-1">Homeowner must explicitly request equipment removal despite wet materials</p>
                </div>
              </label>
              ...
              <label className="flex items-start gap-3 p-3 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                <input
                  type="checkbox"
                  checked={drwAnswers.risksExplained}
                  onChange={(e) => setDrwAnswers({ ...drwAnswers, risksExplained: e.target.checked })}
                  className="w-5 h-5 mt-0.5 rounded border-gray-300"
                />
                <div>
                  <p className="font-medium text-gray-900">Have you explained the risks of mold and secondary damage?</p>
                  <p className="text-xs text-gray-600 mt-1">Tech must clearly communicate risks of removing equipment before materials are dry</p>
                </div>
              </label>
              ...
              <label className="flex items-start gap-3 p-3 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                <input
                  type="checkbox"
                  checked={drwAnswers.waiverWillingness}
                  onChange={(e) => setDrwAnswers({ ...drwAnswers, waiverWillingness: e.target.checked })}
                  className="w-5 h-5 mt-0.5 rounded border-gray-300"
                />
                <div>
                  <p className="font-medium text-gray-900">Is the homeowner willing to sign a waiver?</p>
                  <p className="text-xs text-gray-600 mt-1">Homeowner must agree to sign DRW acknowledging risks</p>
                </div>
              </label>
            </div>
            ...
            <div className="mt-4 pt-4 border-t-2">
              {drwAnswers.homeownerRequesting && drwAnswers.risksExplained && drwAnswers.waiverWillingness ? (
                <div className="bg-yellow-50 border-2 border-yellow-400 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-3">
                    <AlertCircle className="w-5 h-5 text-yellow-700" />
                    <h6 className="font-semibold text-yellow-900">Proceed with Drying Release Waiver (DRW)</h6>
                  </div>
                  <p className="text-sm text-yellow-800 mb-3">
                    All conditions met. You may proceed to create a DRW and pull equipment. Ensure homeowner signs waiver before removing equipment.
                  </p>
                  <Button
                    variant="primary"
                    className="w-full"
                ...
                <div className="bg-red-100 border-2 border-red-500 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h6 className="font-semibold text-red-900">⛔ Do Not Pull Equipment</h6>
                  </div>
                  <p className="text-sm text-red-800 mb-3">
                    Not all conditions are met. Equipment must remain in place until materials are dry.
                  </p>
                  <div className="text-xs text-red-700 space-y-1 bg-red-50 p-3 rounded">
                    <p className="font-medium">Recommended actions:</p>
                    <ul className="list-disc ml-5 space-y-1">
                      <li>Continue drying with equipment in place</li>
                      <li>Check for proper air circulation and dehumidification</li>
                      <li>Investigate hidden moisture sources</li>
                      <li>Consult with PSM before making pull decision</li>
                    </ul>
                  </div>
                </div>
              )}
      ...
      {Object.entries(materialsByRoom).map(([roomId, materials]) => {
        ...
        return (
          <div key={roomId} className="border rounded-lg p-5 bg-white">
            ...
            {/* Wet Materials Category (Default Open) */}
            <details open className="border-2 border-red-300 rounded-lg overflow-hidden">
              <summary className="bg-red-100 px-4 py-3 cursor-pointer hover:bg-red-200 font-medium text-red-900">
                🔴 Wet Materials ({materials.filter(m => !isMaterialDry(m.readings[m.readings.length - 1].moisturePercent, m.dryStandard)).length})
              </summary>
              <div className="p-4 space-y-3 bg-red-50">
                {materials
                  .filter(m => !isMaterialDry(m.readings[m.readings.length - 1].moisturePercent, m.dryStandard))
                  .map((material) => {
            ...
            {/* Dry Materials Category (Default Closed) */}
            <details className="border-2 border-green-300 rounded-lg overflow-hidden mt-3">
              <summary className="bg-green-100 px-4 py-3 cursor-pointer hover:bg-green-200 font-medium text-green-900">
                ✅ Dry Materials ({materials.filter(m => isMaterialDry(m.readings[m.readings.length - 1].moisturePercent, m.dryStandard)).length})
              </summary>
